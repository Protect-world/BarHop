...
```
v1.1  地基：到店打卡 MVP + 用户行为数据（✅ 已完成，发布版本 v1.0.2）
v1.2  成就体系：徽章 + 等级称号 + 个人足迹地图（点亮）← 当前规划
v1.3  社交裂变：好友打卡榜 + 打卡动态 + 成就分享卡片
v2.0  核心玩法：微醺路线（官方/UGC Bar Crawl）+ 酒款集邮
v2.x  运营商业化：酒吧活动、商家认领、好友约酒、多城市
```

> 版本命名约定：迭代代号（v1.1/v1.2…）用于规划管理；发布版本号（v1.0.x）用于小程序上传与微信后台。
> v1.0.2 = 到店打卡 MVP（含地图左边缘修复、首页头像持久化、/health 额度统计修复、分享功能）。
